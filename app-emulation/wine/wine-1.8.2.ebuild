...
LICENSE="LGPL-2.1"
SLOT="0"
IUSE="+abi_x86_32 +abi_x86_64 +abi_x86_x32 +alsa capi cups custom-cflags dos elibc_glibc +fontconfig +gecko gphoto2 gsm gstreamer +jpeg +lcms ldap +mono mp3 ncurses netapi nls odbc openal opencl +opengl osmesa oss +perl pcap pipelight +png prelink pulseaudio +realtime +run-exes s3tc samba scanner selinux +ssl staging test +threads +truetype +udisks v4l vaapi +X +xcomposite xinerama +xml"
REQUIRED_USE="|| ( abi_x86_32 abi_x86_64 abi_x86_x32 )
	elibc_glibc? ( threads )
	mono? ( abi_x86_32 )
	pipelight? ( staging )
	s3tc? ( staging )
...
src_prepare() {
	...
	local PATCHES=(
		...
		"${FILESDIR}"/${PN}-1.8.2-x32-1.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-2.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-2a.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-2b.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-2c.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-3a.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-4a.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-4b.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-5.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-6.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-7.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-8.patch
		"${FILESDIR}"/${PN}-1.8.2-x32-9a.patch
		"${FILESDIR}"/${PN}-1.9.9-x32-10a.patch
	)
if false; then
		"${FILESDIR}"/${PN}-1.8.2-x32-4.patch
fi
	if use staging; then
...
multilib_src_install_all() {
	...
	(use abi_x86_32 || use abi_x86_x32) && pax-mark psmr "${D}"usr/bin/wine{,-preloader} #255055
	use abi_x86_64 && pax-mark psmr "${D}"usr/bin/wine64{,-preloader}

	if use abi_x86_64 && ! (use abi_x86_32 || use abi_x86_x32); then
