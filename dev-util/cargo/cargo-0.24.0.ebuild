...
get_triple() {
	if [[ ${ARCH} = "amd64" ]]; then
		TRIPLE_HOST="x86_64-unknown-linux-musl"
		if use abi_x86_64 ; then
			TRIPLE="x86_64-unknown-linux-musl"
			TRIPLE_CC="x86_64-pc-linux-musl"
		elif use abi_x86_32 ; then
			TRIPLE="i686-unknown-linux-musl"
			TRIPLE_CC="i686-pc-linux-musl"
		else
			die "Unsupported ABI"
		fi
	else
		TRIPLE_HOST="i686-unknown-linux-musl"
		TRIPLE="i686-unknown-linux-musl"
		TRIPLE_CC="i686-pc-linux-musl"
	fi
}
...
DEPEND="
	${COMMON_DEPEND}
	>=virtual/rust-1.9.0[${MULTILIB_USEDEP}]
	dev-util/cmake
	sys-apps/coreutils
	sys-apps/diffutils
	sys-apps/findutils
