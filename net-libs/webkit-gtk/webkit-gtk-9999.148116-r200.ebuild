...
inherit autotools check-reqs eutils flag-o-matic gnome2 pax-utils python-any-r1 ruby-single toolchain-funcs versionator virtualx subversion

MY_P="webkitgtk-${PV}"
DESCRIPTION="Open source web browser engine"
HOMEPAGE="http://www.webkitgtk.org/"
SRC_URI=""
ESVN_REVISION="148116"
ESVN_REPO_URI="https://svn.webkit.org/repository/webkit/trunk"
...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-2.0.4-ia64-malloc.patch
	...
	epatch "${FILESDIR}"/${PN}-9999.148063-remove-execinfo_h.patch

	#epatch "${FILESDIR}"/webkitgtk-1.10-textrels.patch
	#epatch "${FILESDIR}"/webkitgtk-2.0.4-bison-3.0.patch

	epatch "${FILESDIR}"/${PN}-2.1.3-fix-undefined-reference-to-putByIndexBeyondVectorLengthWithoutAttributes.patch

	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		epatch "${FILESDIR}"/${PN}-9999.148063-r200-harfbuzz-hb_icu_script_to_script.patch
	fi

	AT_M4DIR=Source/autotools eautoreconf

	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		epatch "${FILESDIR}"/${PN}-9999.148116-jsc-jit.patch
		epatch "${FILESDIR}"/${PN}-9999.148063-jsc-llint.patch #same as 20121106-rev1
		#epatch "${FILESDIR}"/${PN}-20121106-rev1-jsc-llint.patch

		#epatch "${FILESDIR}"/${PN}-20121106-jsc-jit.patch
		#epatch "${FILESDIR}"/${PN}-20121106-rev0-jsc-llint.patch

		#epatch "${FILESDIR}"/${PN}-20121019-jsc-jit.patch
		#epatch "${FILESDIR}"/${PN}-20121019-jsc-llint.patch
		...
		sed -i -r -e 's|as_fn_error \$\? "Compiler GCC|true; #as_fn_error \$\? "Compiler GCC|g' configure || die

		epatch "${FILESDIR}"/${PN}-9999.148063-glslang_tab.h-header-paths.patch
