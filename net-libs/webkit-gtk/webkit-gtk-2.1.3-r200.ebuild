...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-2.1.3-remove-execinfo_h.patch

	epatch "${FILESDIR}"/${PN}-2.1.3-gtk-is-x11-display-fixed.patch
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		...
		epatch "${FILESDIR}"/${PN}-2.1.3-x32-useX87.patch

#testing
		sed -i '/parse-param/ a%lex-param {YYLEX_PARAM}' \
	             Source/ThirdParty/ANGLE/src/compiler/glslang.y
		epatch "${FILESDIR}"/${PN}-2.0.4-x32-weakcompareandswap.patch
		epatch "${FILESDIR}"/${PN}-2.0.4-isnan.patch #required
		epatch "${FILESDIR}"/${PN}-2.0.4-pluginprocessproxyunix-signalh.patch
		epatch "${FILESDIR}"/${PN}-2.1.3-pluginprocessmainunix-libgen.patch

		epatch "${FILESDIR}"/${PN}-2.1.3-fix-stack-size-musl.patch
