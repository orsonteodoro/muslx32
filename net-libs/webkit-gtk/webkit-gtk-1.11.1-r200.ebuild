...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-1.10.1-remove-execinfo_h.patch
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		...
		#epatch "${FILESDIR}"/${PN}-2.0.4-isnan.patch #does not apply
		#epatch "${FILESDIR}"/${PN}-2.0.4-pluginprocessproxyunix-signalh.patch #does not apply
		epatch "${FILESDIR}"/${PN}-2.0.4-pluginprocessmainunix-libgen.patch

		epatch "${FILESDIR}"/${PN}-1.11.1-stacksize-musl.patch
