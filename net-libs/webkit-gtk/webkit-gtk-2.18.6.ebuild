...
IUSE+=" system-malloc accelerated-overflow-scrolling ftl-jit minibrowser"
...
REQUIRED_USE="
	...
	wayland? ( egl )
	|| ( aqua wayland X )
	elibc_musl? ( !geolocation !introspection )
"
...
RDEPEND="
	>=x11-libs/cairo-1.10.2:=[${MULTILIB_USEDEP}]
	...
	>=media-libs/harfbuzz-1.3.3:=[icu(+),${MULTILIB_USEDEP}]
	...
	ftl-jit? ( >=sys-devel/llvm-3.5.0[${MULTILIB_USEDEP}] )
	...
	opengl? ( virtual/opengl[${MULTILIB_USEDEP}]
		x11-libs/cairo[opengl,${MULTILIB_USEDEP}] )
	webgl? (
		x11-libs/cairo[opengl,${MULTILIB_USEDEP}]
		x11-libs/libXcomposite[${MULTILIB_USEDEP}]
		x11-libs/libXdamage[${MULTILIB_USEDEP}] )
...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-2.18.1-musl.patch
...
#	fi
	...
	if use minibrowser ; then
		mycmakeargs+=( -DENABLE_MINIBROWSER=ON )
	fi
...
multilib_src_install() {
	...
	if use minibrowser ; then
		dosym /usr/$(get_libdir)/misc/webkit2gtk-${VERSION}/MiniBrowser /usr/bin/MiniBrowser
	fi
}
