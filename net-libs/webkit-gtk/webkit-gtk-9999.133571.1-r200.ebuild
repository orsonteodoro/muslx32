...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-9999.133571.1-remove-execinfo_h.patch
	...
	#epatch "${FILESDIR}"/${PN}-1.11.1-processlaunchergtk_cpp-AF_UNIX.patch
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		...
		epatch "${FILESDIR}"/${PN}-9999.133571-jsc-llint.patch #same as 20121106-rev1
