...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-7.5.1-debugreg.patch
	#epatch "${FILESDIR}"/${PN}.patch
	epatch "${FILESDIR}"/${PN}-7.5.1-1.patch
	EPATCH_OPTS="-p1" epatch "${FILESDIR}"/${PN}-7.5.1-2.patch
	epatch "${FILESDIR}"/${PN}-7.5.1-3.patch
	epatch "${FILESDIR}"/${PN}-7.5.1-4.patch
...
src_configure() {
	...
	strip-flags
	filter-flags -O2 -O3 -O4 -O1 -Os -O0
	append-cflags -O2
	append-cxxflags -O2
	append-cppflags -I${S}/gdb
