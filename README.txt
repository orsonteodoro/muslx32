...
What is x32?  x32 ABI is 32-bit (4-bytes) per integer, long, pointer using all of the x86_64 general purpose registers identified as (rax,rbx,rcx,r11-r15) and using sse.  Long-long integers are 8 bytes.  C/C++ programs will use __ILP32__ preprocessor checks to distinguish between 32/64 bit systems.  The build system may also compare void* to see if it has __x86_64__ defined and 4 bytes for 32-bit for 8 bytes for 64-bit for LP64 (longs are 8 bytes as well as pointers).
