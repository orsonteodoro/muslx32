...
src_prepare() {
	...
	#epatch "${FILESDIR}"/xz-5.2.2-musl.patch
	#epatch "${FILESDIR}"/xz-5.2.2-musl2.patch
}
...
multilib_src_configure() {
	...
	ECONF_SOURCE="${S}" econf \
		...
		gl_cv_posix_shell=${SHELL} || echo --disable-{xz,xzdec,lzmadec,lzmainfo,lzma-links,scripts}
}
