...
src_prepare() {
	...
	default
	#eapply_user
#	[[ "${CHOST}" =~ "muslx32" ]] && epatch "${FILESDIR}"/${PN}-4.4.0-sigcontext.patch
	[[ "${CHOST}" =~ "muslx32" ]] && epatch "${FILESDIR}"/${PN}-4.4.0.148-x32-1.patch
	multilib_copy_sources
}

multilib_src_configure() {
	[[ "${CHOST}" =~ "muslx32" ]] && myconfig=( --disable-boehm --without-sigaltstack )
