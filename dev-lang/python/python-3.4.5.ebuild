...
src_install() {
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		#bugged python_moduleinto
		mkdir -p "${ED}"/usr/$(get_libdir)/python${PYVER}/site-packages || die
		cp "${BUILD_DIR}"/epython.py "${ED}"/usr/$(get_libdir)/python${PYVER}/site-packages/ || die
	else
