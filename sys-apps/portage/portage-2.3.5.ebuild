...
PATCHES=( "${FILESDIR}"/${PV}-loff_t.patch ) #from musl overlay
...
python_prepare_all() {
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		epatch "${FILESDIR}"/0000-portage-2.2.28-musl.patch
		epatch "${FILESDIR}"/0001-portage-2.2.28-musl.patch
		epatch "${FILESDIR}"/0002-portage-2.2.28-musl.patch
		epatch "${FILESDIR}"/0003-portage-2.2.28-musl.patch
	fi
