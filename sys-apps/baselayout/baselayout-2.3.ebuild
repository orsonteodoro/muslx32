...
pkg_setup() {
	#is_crosscompile
	#if [[ "${CHOST}" =~ "muslx32" && "$?" != "0" ]] ; then
	#	die "You must emerge with cross compiler."
	#fi
	multilib_layout
}
	...
	for prefix in "${EROOT}"{,usr/,usr/local/} ; do
		if [ "${SYMLINK_LIB}" = yes ] ; then
			...
			if [[ "${CHOST}" =~ "muslx32" ]] ; then
				if [ -h "${prefix}lib" ] ; then
					if [[ $(readlink -f "${prefix}lib") =~ "${prefix}libx32" ]] ; then
						continue
					else
...
pkg_postinst() {
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		if [[ ! $(readlink -f "${EROOT}lib") =~ "${EROOT}libx32" ]] ; then
		        echo "fix broken lib in the crossdev profile? it will move lib to libx32 and symlink libx32 back to lib. (y/n)"
		       	echo "automatically choosing y"
		        X="y"
			if [[ "$X" == "y" ]] ; then
		                if [ -d "${EROOT}"lib ] ; then
		                       	mkdir -p "${EROOT}"libx32
		                        cp -af "${EROOT}"lib/* "${EROOT}"libx32
		                        cd "${EROOT}"
		                        rm -rf "${EROOT}"lib
		                       	ln -s libx32 lib
		                fi

	        	        if [ -d "${EROOT}"libx32 -a ! -L "${EROOT}"lib ] ; then
		                        cd "${EROOT}"
		                        ln -s libx32 lib
		               	fi
		        fi
		        ...
		        X="y"
			if [[ "$X" == "y" ]] ; then
		               	if [ -d "${EROOT}"usr/lib ] ; then
		                        mkdir -p "${EROOT}"usr/libx32
	        	                cp -af "${EROOT}"usr/lib/* "${EROOT}"usr/libx32/
	                	       	cd "${EROOT}"usr
	                        	rm -rf "${EROOT}"usr/lib
		                        ln -s libx32 lib
		                fi

	        	        if [ -d "${EROOT}"usr/libx32 -a ! -L "${EROOT}"usr/lib ] ; then
	                	        cd "${EROOT}"usr
	                       		ln -s libx32 lib
		                fi
        		fi
