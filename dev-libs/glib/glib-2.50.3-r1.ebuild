...
src_prepare() {
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		#gobject introspection fix
		#epatch "${FILESDIR}"/0001-gquark-fix-initialization-with-c-constructors.patch
		epatch "${FILESDIR}"/gobject_init_on_demand.patch 
		epatch "${FILESDIR}"/quark_init_on_demand.patch
	fi
