...
RDEPEND=">=app-text/hunspell-1.2:=
	...
	>=media-libs/libpng-1.6.25:0=[apng,${MULTILIB_USEDEP}]
	...
	system-cairo? ( >=x11-libs/cairo-1.12[X,xcb,${MULTILIB_USEDEP}] >=x11-libs/pixman-0.19.2[${MULTILIB_USEDEP}] )
	system-icu? ( >=dev-libs/icu-58.1:=[${MULTILIB_USEDEP}] )
	system-jpeg? ( >=media-libs/libjpeg-turbo-1.2.1[${MULTILIB_USEDEP}] )
	system-libevent? ( >=dev-libs/libevent-2.0:0=[${MULTILIB_USEDEP}] )
	system-sqlite? ( >=dev-db/sqlite-3.17.0:3[secure-delete,debug=,${MULTILIB_USEDEP}] )
	system-libvpx? ( >=media-libs/libvpx-1.5.0:0=[postproc,${MULTILIB_USEDEP}] )
	system-harfbuzz? ( >=media-libs/harfbuzz-1.3.3:0=[${MULTILIB_USEDEP}] >=media-gfx/graphite2-1.3.8[${MULTILIB_USEDEP}] )
"
...
if [[ -n ${MOZCONFIG_OPTIONAL_GTK3} ]]; then
	...
	RDEPEND+=" force-gtk3? ( >=x11-libs/gtk+-3.4.0:3[${MULTILIB_USEDEP}] )"
	...
	RDEPEND+=" !gtk2? ( >=x11-libs/gtk+-3.4.0:3[${MULTILIB_USEDEP}] )"
fi
