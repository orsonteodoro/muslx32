...
autotools-utils_src_prepare() {
	...
	einfo "Applying automake musl patch"
        for file in $(grep -l -r -e "\$(am__cd) \$\$subdir && \$(MAKE) \$(AM_MAKEFLAGS) \$\$local_target" ./); do
	...
	einfo "Applying libtool musl patch"
        for file in $(grep -l -r -e "\$(INSTALL_DATA) \$\$files" ./); do
		if [[ "$file" =~ "xcbgen/Makefile.in" ]]; then
			echo "xcb found 1"
			continue #xcb-proto
		fi
                sed -i 's|\$(INSTALL_DATA) \$\$files|$(INSTALL_DATA) $(srcdir)/$$files|g' "$file"
		sed -i 's|echo " \$(INSTALL_DATA)|test -z "$$files" \|\| { echo " $(INSTALL_DATA)|g' "$file"
		#
		sed -i 's|" \|\| exit $$?; \\|" \|\| exit $$?; }; \\|g' "$file"
        done
        for file in $(grep -l -r -e "\$(INSTALL_HEADER) \$\$files" ./); do
		if [[ "$file" =~ "xcbgen/Makefile.in" ]]; then
			echo "xcb found 2"
			continue #xcb-proto
		fi
                sed -i 's|\$(INSTALL_HEADER) \$\$files|$(INSTALL_HEADER) $(srcdir)/$$files|g' "$file"
		sed -i 's|echo " \$(INSTALL_HEADER)|test -z "$$files" \|\| { echo " $(INSTALL_HEADER)|g' "$file"
		sed -i 's|" \|\| exit $$?; \\|" \|\| exit $$?; }; \\|g' "$file"
        done
}
