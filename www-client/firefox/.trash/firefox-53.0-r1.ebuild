...
pkg_setup() {
		...
		ewarn "reason: broken javascript compile time bug.  also, fix javascript bug.  fix 49.0 before fixing 53.0."
...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-53.1.0-01-2c924fb664c1.patch
	epatch "${FILESDIR}"/${PN}-53.1.0-02-97c45b13a165.patch
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		...
		if use abi_x86_x32; then
			...
			#epatch "${FILESDIR}"/${PN}-47.0.1-xpcom-x32.patch #testing as disable
...
src_configure() {
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		mozconfig_annotate '' --disable-ion
