...
src_prepare() {
	...
	if [[ "${CHOST}" =~ "muslx32" ]]; then
		...
		#from alpine linux
		#epatch "${FILESDIR}"/unset-madv_free.patch
		#epatch "${FILESDIR}"/${PN}-53.0.2785.143-x32-PartitionAlloc_h-subscript.patch
...
src_configure() {
	...
	#if [[ "${CHOST}" =~ "muslx32" ]] ; then
	#	myconf_gyp+=" -Denable_pdf=1 -Dpdf_enable_v8=0" #old v8 would require some more changes
	#fi
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		...
		myconf_gyp+=" -Dx86_64_abi='x32' -Duse_musl_support=1"
