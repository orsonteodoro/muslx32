...
IUSE="debug cups +gn gnome gnome-keyring gtk3 +hangouts kerberos neon pic +proprietary-codecs pulseaudio selinux +suid +system-ffmpeg tcmalloc widevine"
...
src_prepare() {
	...
	if [[ "${CHOST}" =~ "muslx32" ]]; then
		...
		epatch "${FILESDIR}"/${PN}-54.0.2824.0-x32-3.patch #there is no 54.0.2824.0-x32-{2,1}.patch
		...
		epatch "${FILESDIR}"/${PN}-54.0.2824.0-x32-7.patch
		...
		epatch "${FILESDIR}"/${PN}-54.0.2824.0-x32-15.patch
		epatch "${FILESDIR}"/${PN}-52.0.2743.116-x32-16.patch
		epatch "${FILESDIR}"/${PN}-52.0.2743.116-x32-17.patch
		epatch "${FILESDIR}"/${PN}-54.0.2824.0-x32-18.patch
		epatch "${FILESDIR}"/${PN}-54.0.2824.0-x32-19.patch
		epatch "${FILESDIR}"/${PN}-52.0.2743.116-x32-20.patch
		epatch "${FILESDIR}"/${PN}-52.0.2743.116-x32-21.patch
		epatch "${FILESDIR}"/${PN}-54.0.2824.0-x32-22.patch
		epatch "${FILESDIR}"/${PN}-52.0.2743.116-x32-23.patch
		epatch "${FILESDIR}"/${PN}-54.0.2824.0-x32-24.patch
		epatch "${FILESDIR}"/${PN}-54.0.2824.0-x32-25.patch
	...
	if use debug; then
		...
		epatch "${FILESDIR}"/chromium-54.0.2824.0-better-debug-2.patch
	fi

	EPATCH_SOURCE="${FILESDIR}/musl" EPATCH_SUFFIX="patch" \
	EPATCH_MULTI_MSG="Applying musl patches ..." EPATCH_EXCLUDE="09_all_fix-sctp.patch" epatch
